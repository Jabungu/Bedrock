...
class Poll(models.Model):
    ...
    user = models.ForeignKey(User, related_name="status", on_delete = models.CASCADE, null=True)
    water = models.CharField(max_length=1, choices=choice)
    electricity = models.CharField(max_length=1, choices=choice)
    electricity_provider =models.CharField(max_length=30)
    internet = models.CharField(max_length=1, choices=choice)
...
class Like(models.Model):
    ...
    comment = models.ForeignKey(Comment, related_name="liked_comment", on_delete=models.CASCADE, null=True)
    created = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
class Favorite(models.Model):
    user = models.ForeignKey(User, related_name="favorites", on_delete=models.CASCADE, null=True)
    comment = models.ForeignKey(Comment, related_name="favorited_comments", on_delete=models.CASCADE, null=True)
    created = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
class Connection_request(models.Model):
    ...
    status= models.CharField(max_length=1, choices=choice, default="none")
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
